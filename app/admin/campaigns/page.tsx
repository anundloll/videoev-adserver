...
export default function CampaignAdminPage() {
  ...
  // ── Render ──────────────────────────────────────────────────────────────────
  return (
    <div className="min-h-screen min-w-[960px] bg-slate-950 text-white font-sans flex flex-col">

      {/* Header */}
      <header className="shrink-0 border-b border-slate-800 px-6 py-3.5 flex items-center gap-3">
        <div className="w-2 h-2 rounded-full bg-teal-400 shrink-0" />
        <span className="eyebrow text-slate-500">VideoEV AdCP</span>
        <span className="text-slate-700">/</span>
        <span className="text-sm font-semibold">Campaign Admin</span>
        {loadState === 'ok' && (
          <span className="ml-auto text-xs text-slate-600 num">{campaigns.length} campaigns in DB</span>
        )}
        {loadState === 'error' && (
          <span className="ml-auto text-xs text-red-400">⚠ DB offline — check DATABASE_URL</span>
        )}
        {loadState === 'loading' && (
          <span className="ml-auto text-xs text-slate-600">Loading…</span>
        )}
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── Left: Campaign list ─────────────────────────────────────────── */}
        <aside className="w-60 shrink-0 border-r border-slate-800 flex flex-col overflow-hidden">
          <div className="p-3 border-b border-slate-800 space-y-2">
            <input
              ...
              className="w-full bg-slate-900 border border-slate-800 rounded px-3 py-1.5 text-sm placeholder-slate-600 focus:outline-none focus:border-teal-600"
            />
            <select
              value={sectorFilter}
              onChange={e => setSectorFilter(e.target.value)}
              className="w-full bg-slate-900 border border-slate-800 rounded px-2 py-1.5 text-sm text-slate-300 focus:outline-none focus:border-teal-600"
            >
          ...
          <div className="flex-1 overflow-y-auto">
            {loadState === 'loading' && (
              <div className="p-4 text-xs text-slate-600 text-center">Loading campaigns…</div>
            )}
            {loadState === 'error' && (
            ...
            {filtered.map(c => {
              ...
              return (
                <button
                  key={c.id}
                  onClick={() => setInspected(isActive ? null : c)}
                  className={`w-full text-left px-3 py-2.5 border-b border-slate-800/60 hover:bg-slate-900 transition-colors ${isActive ? 'bg-slate-900 border-l-2 border-l-teal-400 pl-2.5' : ''}`}
                >
                  ...
                  <div className="flex items-center justify-between mt-0.5">
                    ...
                    {c._count.trackingEvents > 0 && (
                      <span className="text-[10px] text-slate-600 num">{c._count.trackingEvents} events</span>
                    )}
                  </div>
                </button>
              );
        ...
        <main className="flex-1 overflow-y-auto p-8">
          <h1 className="text-xl font-semibold mb-1">New Campaign</h1>
          <p className="text-xs text-slate-500 mb-8">
            Saved campaigns go directly into the live auction engine.
          </p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-900 border border-slate-800 rounded px-3 py-2 text-sm placeholder-slate-600 focus:outline-none focus:border-teal-600"
                />
              </div>
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-900 border border-slate-800 rounded px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-teal-600"
                >
                  {SECTORS.map(s => <option key={s}>{s}</option>)}
                </select>
              </div>
              <div>
                ...
                <input
                  ...
                  step="0.5" min="1"
                  className="w-full bg-slate-900 border border-slate-800 rounded px-3 py-2 text-sm focus:outline-none focus:border-teal-600"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  placeholder="Leave blank → S3 placeholder"
                  className="w-full bg-slate-900 border border-slate-800 rounded px-3 py-2 text-sm placeholder-slate-600 focus:outline-none focus:border-teal-600"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Scan for 10% off"
                  className="w-full bg-slate-900 border border-slate-800 rounded px-3 py-2 text-sm placeholder-slate-600 focus:outline-none focus:border-teal-600"
                />
              </div>
            </div>
          ...
          <fieldset className="mb-8">
            <div className="flex items-center justify-between mb-4">
              ...
              <button type="button" onClick={addRule} className="text-xs text-teal-400 hover:text-teal-300 transition-colors">
                + Add Rule
              </button>
            </div>
                ...
                return (
                  <div key={rule.id} className="grid grid-cols-[1fr_1fr_80px_28px] gap-2 items-center">
                    <select
                      ...
                      className="bg-slate-900 border border-slate-800 rounded px-2 py-2 text-sm text-slate-200 focus:outline-none focus:border-teal-600"
                    >
                    ...
                    <select
                      ...
                      className="bg-slate-900 border border-slate-800 rounded px-2 py-2 text-sm text-slate-200 focus:outline-none focus:border-teal-600 disabled:opacity-40"
                    >
                    ...
                    {isAffinity ? (
                      <div className="flex flex-wrap gap-1 py-1">
                        {affinityTags.map(t => (
                          <span key={t} className="text-[9px] bg-teal-900/40 text-teal-300 px-1.5 py-0.5 rounded">{t}</span>
                        ))}
                      </div>
                    ) : (
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          className="w-full bg-slate-900 border border-slate-800 rounded px-2 py-2 text-sm text-center focus:outline-none focus:border-teal-600"
                        />
                    ...
                    <button
                      ...
                      className="text-slate-700 hover:text-red-500 transition-colors text-base leading-none"
                    >
                      ×
                    </button>
                  </div>
            ...
            <p className="text-[11px] text-slate-600 mt-3">
              ...
              MSRP / Venue → injects affinity tags (each tag = +10 auction score).
            </p>
          </fieldset>
          ...
          {/* Error */}
          {saveError && (
            <div className="mb-4 px-3 py-2 bg-red-900/30 border border-red-800 rounded text-xs text-red-300">
              ⚠ {saveError}
            </div>
          )}
          ...
          <button
            ...
            className="w-full py-3 bg-teal-500 hover:bg-teal-400 disabled:opacity-40 disabled:cursor-not-allowed text-slate-950 font-bold text-sm rounded-lg transition-colors"
          >
        ...
        <div className="w-80 shrink-0 border-l border-slate-800 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800 shrink-0">
            <span className="eyebrow text-slate-400">Generated JSON</span>
            {generatedJson && (
              <button onClick={handleCopy} className="text-xs text-teal-400 hover:text-teal-300 transition-colors">
                {copied ? '✓ Copied' : 'Copy'}
              </button>
            )}
          </div>
          ...
          {/* Campaign inspector */}
          {inspected && (
            <div className="border-t border-slate-800 flex flex-col max-h-72 shrink-0">
              <div className="flex items-center justify-between px-4 py-2 border-b border-slate-800/60 shrink-0">
                ...
                <button onClick={() => setInspected(null)} className="text-slate-600 hover:text-slate-400 text-sm shrink-0">×</button>
              </div>
